...
const WorldMapVisualization: React.FC = () => {
  ...
  const handleMarkerClick = useCallback((countryCode: string) => {
  ...
  return (
    <>
      <div className="flex flex-col h-full min-h-0">
        <div className="flex items-center justify-between mb-3 shrink-0">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">Click markers to see visitors by country</p>
          </div>
        ...
        <div className="relative flex-1 w-full border border-zinc-200 rounded-md bg-[#fafafa] overflow-hidden">
            ...
            >
              ...
              {markers.map((marker) => (
                <Marker
                  ...
                  onClick={() => handleMarkerClick(marker.country_code)}
                >
                  {/* Pulse ring */}
                  <circle
                    r={8 / position.zoom}
                  ...
                  {hoveredMarker === marker.country_code && (
                    <g>
                      <rect
                        x={8 / position.zoom}
                        y={-12 / position.zoom}
                        width={(marker.coords.name.length * 6 + 60) / position.zoom}
          ...
          {/* Top locations legend - clickable */}
            ...
            <div className="absolute top-2 right-2 bg-white/95 backdrop-blur-sm border border-zinc-200 rounded px-2 py-1.5 shadow-sm">
              ...
              <div className="space-y-0.5">
                {topLocations.slice(0, 3).map((loc, index) => (
                  <button
                    key={loc.code}
                    onClick={() => handleMarkerClick(loc.code)}
                    className="flex items-center gap-1.5 w-full hover:bg-zinc-100 rounded px-1 -mx-1 transition-colors"
                  >
                    <div 
                    ...
                    <span className="text-[10px] text-zinc-400 tabular-nums ml-auto">{loc.count}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      {/* Country Visitors Modal */}
      <CountryVisitorsModal
